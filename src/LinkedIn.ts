 ...
 */
export class LinkedIn extends OAuthConfig {
  /**
   * The HTTP client.
   */
   ...
   */
  public scopes: string[] = ["r_liteprofile", "r_emailaddress"];
  ...
  /**
   * Get the LinkedIn OAuth configuration as an object.
   */
  getConfig(): any {
    return {
  ...
  getAuthURL(): string {
    ...
    return this.buildUrl(LinkedIn.AUTH_URL, params);
  }
  ...
  async getUser(token: string): Promise<any | null> {
    try {
      const response = await this.client.get(LinkedIn.USER_URL, {
        headers: {
